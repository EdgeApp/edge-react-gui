...
// they can be rotated via getKeys without a store release.
 ...
 * (not inside a plugin map) and merged with the `globalKeys` section a getKeys
 * payload delivers.
 */
 ...
 * bare string) lands in keys so getKeys can rotate every field. Char-code
 * arrays on secret-named fields are still deobfuscated at split time.
 */
 ...
 * dropped; WALLET_CONNECT_INIT moves into pluginApiKeys; POSTHOG_INIT becomes
 * config.POSTHOG_API_HOST + a flat keys.POSTHOG_API_KEY; loose partner secrets
 * (Azteco, Kiln, …) land flat in keys.json; YOLO_* stays in config.
 *
 * Plugin maps in config.json are flag-only (`true` / `false`). Every init
 * object lives in keys.json (`pluginApiKeys` for currency/swap/GUI providers,
 * `rampPlugins` for ramps).
 */
  ...
  for (const [field, value] of Object.entries(env)) {
    ...
    if (coreId != null || swapId != null) {
      ...
      if (coreId != null) {
        ...
        assignKeys(keys.pluginApiKeys, coreId, init)
      }
      if (swapId != null) {
        if (flag !== undefined) config.swapPlugins[swapId] = flag
        assignKeys(keys.pluginApiKeys, swapId, init)
      }
      continue
    }
    ...
    if (field === 'PLUGIN_API_KEYS') {
      for (const [provider, providerValue] of Object.entries(
        ...
        if (flag !== undefined) config.pluginApiKeys[provider] = flag
        assignKeys(keys.pluginApiKeys, provider, init)
      }
      continue
    }
    ...
    if (field === 'WALLET_CONNECT_INIT') {
      const { config: flag, keys: init } = splitPluginValue(value, field)
      if (flag !== undefined) config.pluginApiKeys.walletconnect = flag
      assignKeys(keys.pluginApiKeys, 'walletconnect', init)
      continue
    }
    ...
    if (isSecretTopLevel(field)) {
      ...
      // Legacy env.json used AIRBITZ_API_KEY; the runtime and getKeys auth read
