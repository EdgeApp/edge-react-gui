...
run_capture "$META/login.json" "${CLI[@]}" password-login "$USER" "$PASS"
...
run_capture "$META/rate-from.json" "${CLI[@]}" rates-usd-to-native 90 bitcoin
...
run_capture "$META/rate-to.json" "${CLI[@]}" rates-usd-to-native 90 ethereum
...
run "${CLI[@]}" rates-usd-to-native 500 bitcoin
run "${CLI[@]}" rates-usd-to-native 500 ethereum
...
for p in "${PLUGINS[@]}"; do
  run_capture "$META/q-from-$p.json" "${CLI[@]}" swap-quote "$BTC_ID" "$ETH_ID" "$NATIVE_FROM" from "$p"
  OID=$(node -e '
try {
  const j=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));
  const q=(j.quotes||[])[0];
...
for p in "${PLUGINS[@]}"; do
  run_capture "$META/q-to-$p.json" "${CLI[@]}" swap-quote "$BTC_ID" "$ETH_ID" "$NATIVE_TO" to "$p"
  OID=$(node -e '
try {
  const j=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));
  const q=(j.quotes||[])[0];
...
run_capture "$META/q-all-from.json" "${CLI[@]}" swap-quote "$BTC_ID" "$ETH_ID" "$NATIVE_FROM" from
