...
interface BuildConfigFile {
  ...
  // branch name -> partial config.json / keys.json. Legacy flat `envJson` is
  // ignored with a log (not translated). Legacy `*_INIT` field names are not
  // handled here.
...
function makeCommonPost(buildObj: BuildObj): void {
  ...
  if (buildObj.envJson != null) {
    ...
    const branch = buildObj.repoBranch
    const envJson = buildObj.envJson
    const hasLegacyBranchBlock =
      typeof envJson === 'object' &&
      envJson != null &&
      !Array.isArray(envJson) &&
      Object.prototype.hasOwnProperty.call(envJson, branch)
    if (
      hasLegacyBranchBlock &&
      buildObj.configJson?.[branch] == null &&
      buildObj.keysJson?.[branch] == null
    ) {
      throw new Error(
        `deploy-config.json: migrate envJson["${branch}"] to configJson / keysJson`
      )
    }
  }
