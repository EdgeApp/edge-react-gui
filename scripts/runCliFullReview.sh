...
run account-create account-create "$USER" "$PASS" "$PIN"
...
run logout logout
run password-login password-login "$USER" "$PASS"

# PIN login (after logout)
run logout logout
run pin-login pin-login "$USER" "$PIN"
...
if [ -n "$LOGIN_KEY" ] && [ "$LOGIN_KEY" != "undefined" ]; then
  run key-login key-login "$USER" "$LOGIN_KEY"
else
...
run pin-setup pin-setup 2468
run password-setup password-setup "${PASS}x"
# restore password for later
run password-setup-restore password-setup "$PASS"
run recovery2-setup recovery2-setup "What is your favorite color?" "blue" "What is your pet name?" "fluffy"

# --- Wallets ---
section "Wallets"
run wallet-create wallet-create wallet:bitcoin "Review BTC"
...
if [ -n "$WID" ]; then
  ...
  run wallet-rename wallet-rename "$WID" "Renamed BTC"
...
" 2>/dev/null | tail -1)
  ...
  if [ -n "$ADDR" ]; then
    run max-spendable max-spendable "$WID" "$ADDR"
    run spend-dry spend "$WID" "$ADDR" 1000 --dry-run
    run spend-max-dry spend-max "$WID" "$ADDR" --dry-run
  fi
  run key-list key-list
  run key-get key-get "$WID"
  run wallet-archive wallet-archive "$WID"
  run wallet-unarchive wallet-unarchive "$WID"
fi

# --- Data store ---
section "Data store"
run data-store-set data-store-set reviewStore item1 "hello-cli-review"
run data-store-list data-store-list
run data-store-list-items data-store-list reviewStore
run data-store-get data-store-get reviewStore item1
run data-store-delete data-store-delete reviewStore item1
