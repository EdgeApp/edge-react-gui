...
const MISSING_SECRET_MESSAGE =
  ...
  'Every native build needs the real secret; EDGE_API_SIGNER_ALLOW_STUB=1 is only for `npm run prepare`.'
...
function cByteArray(name: string, bytes: Buffer): string {
  const body = Array.from(bytes)
    ...
    .join(', ')
  // `volatile` is load-bearing, not decoration. Without it clang -O2
  // constant-folds the whole XOR chain: the shard and decoy arrays are dropped
  // entirely (no `__const` section is emitted) and a single 32-byte literal
  // equal to `secret XOR sha256(bundleId)` is left in `__literal16`. Marking
  // them volatile forces every byte to be read at runtime, so the shards
  // survive as distinct symbols and the reassembly actually happens.
  return `static volatile const unsigned char ${name}[${bytes.length}] = { ${body} };\n`
}
...
function makeSource(apiKey: string, secret: Buffer, bundleId: string): string {
  ...
  // Decoy arrays (same shape). They are `volatile const` like the shards and
  // are read at runtime below, so -O2 cannot drop them.
...
/* Opaque accessors so the arrays are not one contiguous blob in .rodata order
 * alone. Byte loops rather than memcpy: memcpy from a volatile array is
 * ill-typed, and the per-byte volatile reads are what defeat constant folding. */
#define EDGE_API_LOAD(fn, arr)                                   \\
  static void fn(uint8_t *o) {                                   \\
    size_t i;                                                    \\
    for (i = 0; i < EDGE_API_SECRET_LEN; ++i) o[i] = arr[i];     \\
  }
EDGE_API_LOAD(ea_load0, ea_s0)
EDGE_API_LOAD(ea_load1, ea_s1)
EDGE_API_LOAD(ea_load2, ea_s2)
EDGE_API_LOAD(ea_load3, ea_s3)
EDGE_API_LOAD(ea_load4, ea_s4)
EDGE_API_LOAD(ea_load5, ea_s5)
...
/**
 * The Node header additionally publishes the runtime pad, so
 * edge_api_signer_napi.c unpads with exactly the id the shards were built
 * against instead of repeating the literal.
 */
function makeNodeHeader(nodeBundleId: string): string {
  if (!/^[\x20-\x7e]+$/.test(nodeBundleId)) {
    throw new Error('nodeBundleId must be printable ASCII')
  }
  return `/* auto-generated by scripts/makeApiSigner.ts — do not edit */
#ifndef EDGE_API_SECRET_GEN_H
#define EDGE_API_SECRET_GEN_H
#include "edge_api_sign.h"
#define EDGE_NODE_BUNDLE_ID ${JSON.stringify(nodeBundleId)}
#endif
`
}
...
function main(): void {
  ...
  const nodeBundleId = NODE_API_SIGNER_BUNDLE_ID
  ...
  try {
    ...
    const missing =
      ...
      String(err.message).includes('edgeKey.json')
    if (!missing) throw error
    // A missing edgeKey.json follows the stub / fail-closed policy below.
    console.log(
  ...
  if (secretHex === '') {
    ...
    // placeholder in Authorization / getApiKey, so refuse that pairing.
    throw new Error(
  ...
  if (!missingSecret) {
    ...
    writeFile(NODE_HEADER, makeNodeHeader(nodeBundleId))
  }
