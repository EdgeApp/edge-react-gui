...
**CLI:** `edge-cli otp-reset-request <username> <resetToken>`
...
**CLI:** `edge-cli recovery2-questions <key> <username>`
...
**CLI:** `edge-cli password-login <username> <password> [<otp>]`
...
**CLI:** `edge-cli pin-login <username> <pin>`
...
**CLI:** `edge-cli key-login <username> <account-key>`
...
**CLI:** `edge-cli recovery2-login <key> <username> <answers…>`
...
**CLI:** `edge-cli account-create <username> <password> <pin>`
...
**CLI:** `edge-cli password-setup <password>`
...
**CLI:** `edge-cli recovery2-setup [<question> <answer>]…`
...
**CLI:** `edge-cli otp-enable [<timeout>]`
...
**CLI:** `edge-cli key-add '<key-info-json>'`
...
- `edge-cli wallet-undelete <walletId>`
- `edge-cli key-undelete <walletId>`
...
**CLI:** `edge-cli data-store-get <storeId> <itemId>`
...
**CLI:** `edge-cli data-store-set <storeId> <itemId> <text>`
...
**CLI:** `edge-cli data-store-delete <storeId> <itemId>`
...
**CLI:** `edge-cli wallet-rename <walletId> <newName>` (and related flags for
fiat / pause when implemented)
...
**CLI:** `edge-cli balance <walletId> [<tokenId>]`
...
**CLI:** `edge-cli tx-list <walletId> [<tokenId>] [<limit>] [<startDate>] [<endDate>] [<search>]`
...
**CLI:** `edge-cli max-spendable <walletId> <address> [<tokenId>]`
...
- `edge-cli spend <walletId> <address> <amount> [<tokenId>] [--dry-run]`
- `edge-cli spend-max <walletId> <address> [<tokenId>] [--dry-run]`
...
`{ "to", "amount", "tokenId" }` (`to` is `parseUri`'d like spend).
...
**CLI:** `edge-cli make-spend <walletId> <address> <amount> [<tokenId>]`
...
**CLI:** `edge-cli sign-tx <walletId> <objectId>`
...
**CLI:** `edge-cli broadcast-tx <walletId> <objectId>`
...
**CLI:** `edge-cli save-tx <walletId> <objectId>`
...
**CLI:** `edge-cli rates-query '{"crypto":[{"pluginId":"bitcoin"}]}'`
...
**CLI:** `edge-cli rates-usd-to-native 90 bitcoin`
...
**CLI:**
`edge-cli swap-quote <fromWid> <toWid> <nativeAmount> [from|to|max] [<pluginId>]`
...
**CLI:** `edge-cli token-enable <walletId> <tokenId>`
...
**CLI:** `edge-cli token-disable <walletId> <tokenId>`
