...
- GET `/gift-cards/:country` → list gift card brands/products by country
...
- `GiftCardIdentityFormScene`
  ...
  - On submit: calls `provider.registerUser`, persists identity (including `userApiKey`), updates provider, routes to `GiftCardMarketScene`.
