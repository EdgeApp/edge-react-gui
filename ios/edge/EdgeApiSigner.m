                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  ...
  // `NSData.bytes` is NULL for a zero-length NSData, which would trip the
  // `message == NULL` guard in edge_api_hmac_sign and reject. Android signs the
  // empty message happily, so point at a valid zero-length buffer to keep the
  // two platforms byte-compatible.
  const uint8_t *msgBytes =
    msgData.length == 0 ? (const uint8_t *)"" : (const uint8_t *)msgData.bytes;
