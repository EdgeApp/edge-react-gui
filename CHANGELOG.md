...
- changed: (ARRR) Replace `react-native-piratechain` with `react-native-pirate-wallet` 0.3.4, the Pirate Chain team's unified wallet SDK. It carries the Ironwood shielded pool and the consensus rules Ironwood activation brings, which the old SDK does not, and it retires the two crash families the old module produced. Pirate Chain wallets also now scan and broadcast over TLS, against a node that cold-syncs a year-old wallet in about a minute, keep their registry credential in the iOS Keychain or Android Keystore, and lock their signing keys whenever the account is not running them.
- fixed: A send whose funds are not spendable yet now says so, instead of showing "An unexpected error occurred. Please check your network connection." This is what a Pirate Chain wallet reports while it finishes preparing to spend after a sync, and what a Monero wallet reports while its balance is still unconfirmed.
