...
- changed: (ARRR) Replace `react-native-piratechain` with `react-native-pirate-wallet`, the Pirate Chain team's unified wallet SDK. It carries the Ironwood shielded pool and the consensus rules Ironwood activation brings, which the old SDK does not.
