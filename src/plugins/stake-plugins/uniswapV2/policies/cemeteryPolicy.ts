...
export const makeCemeteryPolicy = (
  options: CemeteryPolicyOptions
): StakePluginPolicy => {
  // Declare contracts:
  const {
    disableStake = false,
  ...
  const instance: StakePluginPolicy = {
    async fetchChangeQuote(request: ChangeQuoteRequest): Promise<ChangeQuote> {
      ...
      if (action === 'claim' || action === 'unstake') {
        const rewardNativeAmount = (
          await eco.multipass(p =>
            poolContract.connect(p).pendingShare(POOL_ID, signerAddress)
          )
        ).toString()
        allocations.push(
            ...
            txs.build(
              (gasLimit =>
                async function approveSwapRouter({ signer }) {
                  const allowanceResult = await tokenAContract.allowance(
                    signer.address,
                    spenderAddress
                  )
        ...
        txs.build(
          ((gasLimit, lpTokenBalance) =>
            async function addLiquidity({ signer }) {
              ...
              const liquidityDiffAmount = sub(
                expectedLiquidityAmount,
                lpTokenBalance
              )
              const assetAmountsFromLpDifference =
                await lpTokenToAssetPairAmounts(policyInfo, liquidityDiffAmount)
              const tokenAAmountFromLpDifference =
                assetAmountsFromLpDifference[serializeAssetId(tokenAAllocation)]
                  .nativeAmount
              const tokenBAmountFromLpDifference =
                assetAmountsFromLpDifference[serializeAssetId(tokenBAllocation)]
                  .nativeAmount

              // Prepare the contract parameters
              const amountTokenADesired = tokenAAmountFromLpDifference
              const amountTokenAMin = round(
                mul(tokenAAmountFromLpDifference, SLIPPAGE_FACTOR.toString())
              )
              const amountTokenBDesired = tokenBAmountFromLpDifference
              const amountTokenBMin = round(
                mul(tokenBAmountFromLpDifference, SLIPPAGE_FACTOR.toString())
              )
      ...
      */
      if (action === 'unstake') {
        ...
        const userInfo = await eco.multipass(p =>
          poolContract.connect(p).userInfo(POOL_ID, signerAddress)
        )
        ...
        txs.build(
          ((gasLimit, lpTokenBalance) =>
            async function unstakeLiquidity({ signer }) {
              ...
              const result = await poolContract
                .connect(signer)
                .withdraw(POOL_ID, amountToUnstake, {
                  gasLimit,
                  gasPrice,
                  nonce: nextNonce()
                })
    ...
    async fetchStakePosition(
      ...
      const [
        { stakedLpTokenBalance, assetAmountsFromLp },
        rewardNativeAmount,
        tokenABalance,
        tokenBBalance,
        lpTokenBalance
        ...
        eco
          .multipass(p => lpTokenContract.connect(p).balanceOf(signerAddress))
          .then(String)
      ])
      ...
      const canUnstakeAndClaim =
        !disableUnstake && gt(stakedLpTokenBalance, '0')
