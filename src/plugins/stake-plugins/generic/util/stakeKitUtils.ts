...
// remote secrets asynchronously and mutates `globalKeys` in place, so anything read
// while this module is first evaluated is the baked-in fallback forever.
const makeHeaders = (): Record<string, string> => ({
  'Content-Type': 'application/json',
  'X-API-KEY': globalKeys.STAKEKIT_API_KEY ?? ''
})
...
export const actionEnter = async (
  ...
  if (actionRequestDto.args.validatorAddress != null) {
    const validators = await fetch(
      `${baseUrl}/v1/yields/${actionRequestDto.integrationId}/validators`
    )
    const json: ValidatorDto[] = await validators.json()
    const validator = json.find(
      validator => validator.address === actionRequestDto.args.validatorAddress
    )
  ...
  const out = await fetchPost<ActionRequestDto, ActionDto>(
    '/v1/actions/enter',
    actionRequestDto
  )
...
export const actionPending = async (
  pendingActionRequestDto: PendingActionRequestDto
): Promise<ActionDto> => {
  const out = await fetchPost<PendingActionRequestDto, ActionDto>(
    '/v1/actions/pending',
    pendingActionRequestDto
  )
  checkInsufficientFunds(out)
...
export const transactionConstruct = async (
  transactionId: string,
  constructTransactionRequestDto: ConstructTransactionRequestDto
): Promise<TransactionDto> => {
  return await fetchPatch<ConstructTransactionRequestDto, TransactionDto>(
    `/v1/transactions/${transactionId}`,
    constructTransactionRequestDto
  )
}
export const transactionSubmitHash = async (
  transactionId: string,
...
export const yieldGetSingleYieldBalances = async (
  integrationId: string,
  yieldBalanceRequestDto: YieldBalanceRequestDto
): Promise<YieldBalanceDto[]> => {
  return await fetchPost<YieldBalanceRequestDto, YieldBalanceDto[]>(
...
const checkInsufficientFunds = (res: unknown): void => {
  const maybeStakeKitInsufficientFundsError = asMaybe(
    asObject({ message: asValue('InsufficientFundsError') })
  )(res)
...
const checkForError = (res: unknown): void => {
  const error = asMaybe(
    asObject({ message: asString, type: asString, code: asNumber })
  )(res)
  if (error != null) {
