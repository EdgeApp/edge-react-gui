import { asArray, asNumber, asObject, asOptional, asString, asValue } from 'cleaners'
...
async function fetchRevolut(endpoint: string, init?: RequestInit) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch Revolut ${endpoint}: ${response.status} - ${text}`)
  }
...
const asRevolutConfig = asObject({
  ...
  payment_methods: asArray(asValue('card', 'revolut', 'apple-pay', 'google-pay'))
})
...
export async function fetchRevolutQuote(params: RevolutQuoteParams): Promise<PartnerQuote> {
  ...
  const data = await fetchRevolut(`/partners/api/2.0/quote?${urlParams.toString()}`)
...
export async function fetchRevolutRedirectUrl(params: RevolutRedirectUrlParams): Promise<RevolutRedirectUrl> {
  const urlParams = new URLSearchParams()
  urlParams.set('fiat', params.fiat)
  ...
  const data = await fetchRevolut(`/partners/api/2.0/buy?${urlParams.toString()}`)
