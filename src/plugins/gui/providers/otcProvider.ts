...
const providerId = 'otc'
const storeId = 'money.otc'
const partnerIcon = 'otc.png'
const pluginDisplayName = 'OTC'
const providerDisplayName = 'OTC'
const supportEmail = 'support@otc.money'

const urls = {
  api: {
    prod: 'https://api.otc.money',
    test: 'https://test-api.otc.money'
  },
  widget: {
    prod: 'https://app.otc.money',
    test: 'https://sandbox--otc.netlify.app'
  }
}

const MODE = ENV.ENABLE_FIAT_SANDBOX ? 'test' : 'prod'

// https://api.otc.money/v1/ramp/blockchains

// Maps Edge pluginIds to OTC blockchain.origin values
...
export const otcProvider: FiatProviderFactory = {
  providerId,
  storeId,
    ...
    const out: FiatProvider = {
      ...
      getSupportedAssets: async ({ direction, paymentTypes, regionCode }): Promise<FiatProviderAssetMap> => {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Error fetching otc blockchains: ${text}`)
        }
      ...
      getQuote: async (params: FiatProviderGetQuoteParams): Promise<FiatProviderQuote> => {
        ...
        // Return a fixed rate for all requested amounts
        const fixedRate = 0.1 // Example fixed rate
        const fiatAmount = (parseFloat(exchangeAmount) * fixedRate).toString()
        const cryptoAmount = exchangeAmount
        ...
        const paymentQuote: FiatProviderQuote = {
          ...
          approveQuote: async (approveParams: FiatProviderApproveQuoteParams): Promise<void> => {
            ...
            console.log('Launching OTC webview url=' + url.href)

            // If OTC needs to launch the Plaid bank linking widget, it needs it in an external
            // webview to prevent some glitchiness. When needed, OTC will send an onMessage
            // trigger with the url to open. The below code is derived from OTC's sample code
            const onMessage = (data: string) => {
              datelog(`**** OTC onMessage ${data}`)
                ...
                onUrlChange: async newUrl => {
                  ...
                  if (isHex(orderId)) {
                    ...
                    try {
                      ...
                      if (!response.ok) {
                        const text = await response.text()
                        console.warn(`Error fetching otc blockchains: ${text}`)
                        return allowedCurrencyCodes
                      }
                      const result = await response.json()
                      ...
                      console.log(`Creating OTC payment`)
