...
export const revolutProvider: FiatProviderFactory = {
  ...
  makeProvider: async (params: FiatProviderFactoryParams): Promise<FiatProvider> => {
    ...
    const provider: FiatProvider = {
      ...
      partnerIcon,
      pluginDisplayName,
      getSupportedAssets: async (params: FiatProviderGetSupportedAssetsParams): Promise<FiatProviderAssetMap> => {
        const region =
      ...
      getQuote: async (params: FiatProviderGetQuoteParams): Promise<FiatProviderQuote> => {
        ...
        const revolutFiat = asMaybe(asRevolutFiat)(supportedAssets.getFiatInfo(fiatKey))
        const cryptoKey = `${params.pluginId}:${params.tokenId}`
        const revolutCrypto = asMaybe(asRevolutCrypto)(supportedAssets.getCryptoInfo(cryptoKey))
        ...
        return {
          ...
          approveQuote: async (approveParams: FiatProviderApproveQuoteParams) => {
            const { showUi, coreWallet } = approveParams
            const walletAddresses = await coreWallet.getAddresses({ tokenId: params.tokenId })
            ...
            const successReturnURL = encodeURIComponent('https://return.edge.app/fiatprovider/buy/revolut?transactionStatus=success')

            const orderId = await io.makeUuid()

            const { ramp_redirect_url: redirectUrl } = await fetchRevolutRedirectUrl({
              fiat: revolutFiat.currency,
            ...
            await showUi.openExternalWebView({
              ...
              async deeplinkHandler(link) {
                if (link.direction === 'sell') {
                  throw new Error('Unexpected sell return link. Sell not supported by Revolut.')
                }
                ...
                if (transactionStatus === 'success') {
                  ...
                  const message =
                    sprintf(lstrings.fiat_plugin_buy_complete_message_s, cryptoAmount, params.displayCurrencyCode, fiatAmount, params.fiatCurrencyCode, '1') +
                    '\n\n' +
...
function addRevolutCrypto(supportedAssets: ProviderSupportStore, getTokenIdFromContract: FiatProviderGetTokenIdFromContract, crypto: RevolutCrypto): void {
  ...
  if (crypto.smartContractAddress != null) {
    tokenId = getTokenIdFromContract({ pluginId, contractAddress: crypto.smartContractAddress })
  } else {
  ...
  supportedAssets.add.direction('*').region('*').fiat('*').payment('*').crypto(cryptoKey)
  supportedAssets.addCryptoInfo(cryptoKey, crypto)
}

function addRevolutPaymentMethod(supportedAssets: ProviderSupportStore, method: string): void {
  switch (method) {
    case 'revolut':
      supportedAssets.add.direction('*').region('*').fiat('*').payment('revolut')
      return
    case 'card':
    case 'apple-pay':
    case 'google-pay':
...
function addRevolutFiat(supportedAssets: ProviderSupportStore, fiat: RevolutFiat) {
