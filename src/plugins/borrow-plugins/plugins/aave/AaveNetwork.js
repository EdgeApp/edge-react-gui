...
export const makeAaveNetworkFactory = (blueprint: AaveNetworkBlueprint): AaveNetwork => {
  ...
  const instance: AaveNetwork = {
    ...
    async getReserveTokenRates(tokenAddress) {
      ...
      const variableApr = parseFloat(div(variableBorrowRate.toString(), RAY_STR, MAX_FLOAT_PRECISION))
      const stableApr = parseFloat(div(stableBorrowRate.toString(), RAY_STR, MAX_FLOAT_PRECISION))
