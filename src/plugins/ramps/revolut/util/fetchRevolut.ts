...
interface FetchRevolutOptions {
  apiKey: string
  /**
   * The base URL to use for Revolut API requests.
   * Production: https://ramp-partners.revolut.com
   * Testing: https://ramp-partners.revolut.codes
   **/
  baseUrl: string
}

const fetchRevolut = (options: FetchRevolutOptions) => {
  const { apiKey, baseUrl } = options
  return async (endpoint: string, init?: RequestInit): Promise<any> => {
    const url = `${baseUrl}${endpoint}`
    const response = await fetch(url, {
      method: 'GET',
      ...init,
      headers: {
        // The RequestInit type is very hard to work with, if you've won
        // the metal as a type-gymnast, then be my guest to fix it.
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch Revolut ${url}: ${response.status} - ${text}`
      )
    }
...
export async function fetchRevolutConfig(
  options: FetchRevolutOptions
): Promise<RevolutConfig> {
  const data = await fetchRevolut(options)('/partners/api/2.0/config')
...
export async function fetchRevolutQuote(
  ...
  const data = await fetchRevolut(options)(
