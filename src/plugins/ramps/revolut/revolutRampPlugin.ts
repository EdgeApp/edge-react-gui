...
export const revolutRampPlugin: RampPluginFactory = (
  ...
  async function fetchProviderConfig(): Promise<RevolutConfig> {
    ...
    // Note: The fetchRevolutConfig function uses ENV.PLUGIN_API_KEYS.revolut?.apiKey internally
    // but we validate that apiKey exists via asInitOptions
    const config = await fetchRevolutConfig()
