...
// Read a plugin init from the pluginMaps map, falling back to a default
// enablement value when the plugin is absent from the config/keys files.
const coreInit = (
  id: string,
  fallback: boolean | JsonObject = false
): boolean | JsonObject =>
  (pluginMaps.corePlugins as EdgeCorePluginsInit)[id] ?? fallback

function buildAccountbasedPlugins(): EdgeCorePluginsInit {
  return {
 ...
 */
function buildUtxoPlugins(): EdgeCorePluginsInit {
  return {
...
export let utxoPlugins: EdgeCorePluginsInit = buildUtxoPlugins()
