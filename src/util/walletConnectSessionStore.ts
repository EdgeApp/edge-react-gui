...
export const readActiveSessionWallets = async (
  ...
  return await queueStoreUpdate(async () => {
    ...
    // Only rewrite the file when something actually fell out of it. The prune
    // is housekeeping, so a failed write must not take down the callers that
    // route requests and list connections: the pruned map they asked for is
    // correct either way, and the stale entries are pruned on the next read.
    const storedCount = Object.keys(sessionWallets).length
    if (Object.keys(activeWallets).length !== storedCount) {
      await writeSessionWallets(account, activeWallets).catch(
        (error: unknown) => {
          console.log(
            'walletConnect readActiveSessionWallets prune error',
            String(error)
          )
        }
      )
