...
function applyPublicRollup(raw: unknown): void {
  ...
  runOnce('checkAppVersion', checkAppVersion).catch(() => {
    // checkAppVersion reports its own failures.
  })
}
 ...
 * signed fetch is normally still in flight rather than failed, so it cannot
 * distinguish the two. Attaching only to the cold-start path would miss the
 * warm launch, which is the common case on every launch after the first.
 */
async function fetchKeys(): Promise<FetchedKeys | null> {
  ...
  // COLD_TOTAL_TIMEOUT_MS, so blocking on a second network call here would
  // spend the keys budget on the public rollup and push boot past the gate.
  if (infoServerData.rollup == null) {
    fetchPublicRollup().catch(() => {
      // fetchPublicRollup reports its own failures.
    })
  }
  return result
}
