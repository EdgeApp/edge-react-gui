 ...
 *
 * The split is on the FIRST underscore, so every later underscore stays in the
 * token id. Token ids need that: a Sui coin type is its address, module and
 * name run together, and Move module names allow underscores. No underscore at
 * all means the chain's mainnet asset. Like the payment-type pin above, these
 * links are authored by partners and marketing, so an empty or malformed spec
 * degrades to no pre-selection rather than dead-ending the whole link.
 */
