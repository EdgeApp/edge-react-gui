...
export const sendLogs =
  (text: string) => async (dispatch: Dispatch, getState: GetState) => {
    ...
    if (account.loggedIn === true) {
      ...
      for (const walletId of Object.keys(currencyWallets)) {
        ...
        for (let i = 0; i < codes.length; i++) {
          const txs = await currencyWallets[walletId].getNumTransactions({
...
`
    ...
    return LOGGER.log('SENDING LOGS WITH MESSAGE: ' + text)
      ...
      .catch(e => {
        throw new Error(
          `${s.strings.settings_modal_send_logs_failure} code ${e.message}`
        )
      })
  }
