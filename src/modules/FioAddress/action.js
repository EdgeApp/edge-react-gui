...
export const createFioWallet =
  ...
  (
    dispatch: Dispatch,
    getState: GetState
  ): Promise<EdgeCurrencyWallet | any> => {
    const fiatCurrencyCode = getDefaultIsoFiat(getState())
    return dispatch(
      createCurrencyWallet(
        s.strings.fio_address_register_default_fio_wallet_name,
        Constants.FIO_WALLET_TYPE,
        fiatCurrencyCode,
        false,
        false
      )
  ...
  async (dispatch: Dispatch, getState: GetState) => {
    ...
    if (fioWallets != null) {
      for (const wallet of fioWallets) {
        ...
        fioAddresses = [
          ...fioAddresses,
          ...walletFioAddresses.map(({ name, expiration }) => ({
            name,
            expiration,
            walletId
          }))
    ...
    if (checkExpired) {
      const expired: Array<FioAddress | FioDomain> = getExpiredSoonFioNames(
        fioAddresses,
        fioDomains
      )

      for (const item: FioAddress | FioDomain of expired) {
        const fioWallet: EdgeCurrencyWallet | void = fioWallets.find(
          (walletItem: EdgeCurrencyWallet) => walletItem.id === item.walletId
        )
        if (!fioWallet) continue
