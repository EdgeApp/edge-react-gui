...
import { View, TouchableHighlight, ActivityIndicator, Image, Platform } from 'react-native'
...
import { border as b, cutOffText, truncateDecimals, decimalOrZero, mergeTokensRemoveInvisible } from '../../../../../utils.js'
import { selectWallet, getEnabledTokens } from '../../../../Wallets/action.js'
...
class FullWalletRow extends Component<Props, State> {
  ...
  render () {
    return <View>{this.props.data.item.id ? <FullWalletListRowConnect settings={this.props.settings} data={this.props.data} customTokens={this.props.customTokens} /> : <FullListRowEmptyData />}</View>
  }
}
  ...
  render () {
    ...
    return (
      <View style={[{ width: platform.deviceWidth }, b()]}>
        <View>
          <TouchableHighlight style={[styles.rowContainer]} underlayColor={styleRaw.walletRowUnderlay.color} {...this.props.sortHandlers} onPress={() => this._onPressSelectWallet(id, currencyCode)}>
                ...
                {Platform.OS === 'ios' && (
                  <View style={[styles.rowNameTextWrapIOS, b()]}>
                    <T style={[styles.rowNameText, b()]} numberOfLines={1}>
                      {symbolImageDarkMono && <Image style={[styles.rowCurrencyLogoIOS, b()]} transform={[{ translateY: 6 }]} source={{ uri: symbolImageDarkMono }} />} {cutOffText(name, 34)}
                    </T>
                  </View>
                )}
                {Platform.OS === 'android' && (
