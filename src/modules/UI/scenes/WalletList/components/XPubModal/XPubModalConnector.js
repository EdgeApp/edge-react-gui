...
const mapStateToProps = (state: State) => {
  const visibleModalNameInput = Constants.VIEW_XPUB_KEY_VALUE
  const visibleModalNameOutput = VISIBLE_MODAL_NAME(visibleModalNameInput)

  const xPubKeySyntax = state.ui.scenes.walletList.xPubKeySyntax
  return {
...
const mapDispatchToProps = (dispatch: Dispatch) => {
  const input = Constants.VIEW_XPUB_KEY_VALUE
