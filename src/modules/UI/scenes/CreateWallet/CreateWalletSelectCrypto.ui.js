...
import { Alert, View, Keyboard, TouchableHighlight, Image } from 'react-native'
...
export class CreateWalletSelectCryptoComponent extends Component<CreateWalletSelectCryptoComponentProps, State> {
  ...
  render () {
    const filteredArray = this.props.supportedWalletTypes.filter(entry => {
      return entry.label.toLowerCase().indexOf(this.state.searchTerm.toLowerCase()) >= 0 || entry.currencyCode.toLowerCase().indexOf(this.state.searchTerm.toLowerCase()) >= 0
    })
        ...
        <View style={styles.scene}>
          ...
          <View style={styles.view}>
            ...
            <View style={[styles.buttons]}>
              ...
              <PrimaryButton style={[styles.next]} onPressFunction={this.onNext} text={s.strings.string_next_capitalized} />
            </View>
          </View>
        </View>
      </SafeAreaView>
