...
export class LTCSettings extends Component {
  render () {
    return (
          ...
          <View style={[s.headerTextWrap, b('yellow')]}>
            <View style={s.leftArea}>
              ...
              <T style={s.headerText}>
                {sprintf(strings.enUS['settings_denomination_title_ltc'])}
              </T>
            </View>
          </View>
        </LinearGradient>
        <RadioRows style={b('green')} onSelect={(denomination) => this.selectDenomination('LTC', denomination)}>
          {
            this.props.denominations.map((denomination) => {
              ...
              const onPress = () => this.props.selectDenomination('LTC', key)
...
const mapStateToProps = (state) => ({
  denominations: SETTINGS_SELECTORS.getDenominations(state, 'LTC'),
  selectedDenominationKey: SETTINGS_SELECTORS.getDisplayDenominationKey(state, 'LTC')
})
const mapDispatchToProps = (dispatch) => ({
  selectDenomination: (currencyCode, denominationKey) => { dispatch(setDenominationKeyRequest(currencyCode, denominationKey)) },
})
export default connect(mapStateToProps, mapDispatchToProps)(LTCSettings)
