...
export default class AutoLogoutModal extends Component {
  ...
  render () {
    ...
    const logoutPeriodOptions = [{ label: SECONDS_TEXT, value: 'seconds' }, { label: MINUTEST_TEXT, value: 'minutes' }, { label: HOURS_TEXT, value: 'hours' }, { label: DAYS_TEXT, value: 'days' }]
    ...
    const measurementPicker = (
      <Picker style={styles.autoLogoutPickerContainer} selectedValue={this.state.timeMeasurement} onValueChange={measurement => this.setState({ timeMeasurement: measurement })}>
    ...
    const modalBottom = <ModalButtons onDone={() => this.onDone({ timeMeasurement: this.state.timeMeasurement, timeNumber: this.state.timeNumber })} onCancel={this.onCancel} />
