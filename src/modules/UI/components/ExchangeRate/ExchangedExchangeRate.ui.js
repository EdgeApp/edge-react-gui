...
export default class ExchangedExchangeRate extends Component<Props> {
  ...
  render () {
    ...
    const secondaryDisplayAmount = this.isBits(primaryCurrencyInfo) ? (parseFloat(this.getSecondaryDisplayAmount()) * 1000).toString() : this.getSecondaryDisplayAmount()

    return <ExchangeRate primaryDisplayAmount={primaryDisplayAmount} primaryInfo={primaryCurrencyInfo} secondaryDisplayAmount={secondaryDisplayAmount} secondaryInfo={secondaryCurrencyInfo} />
  }
}
