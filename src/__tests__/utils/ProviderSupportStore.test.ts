...
describe('ProviderSupportStore', () => {
  ...
  it('isSupported -> true', () => {
    ...
    expect(
      generalStore.is.direction('buy').region('US').fiat('iso:USD').supported
    ).toBe(true)
    expect(
      generalStore.is
        .direction('buy')
  ...
  it('isSupported -> false', () => {
    ...
    expect(
      generalStore.is.direction('sell').region('US').fiat('iso:USD').supported
    ).toBe(false)
    expect(
      generalStore.is
        .direction('sell')
    ...
    expect(
      generalStore.is.direction('buy').region('*').fiat('iso:JPY').supported
    ).toBe(false)
    expect(
      generalStore.is
        .direction('*')
  ...
  it('special matching rules', () => {
    ...
    // all rule -> true
    expect(
      store.is.direction('buy').region('US:CA').fiat('iso:USD').supported
    ).toBe(true)
    expect(
      store.is.direction('buy').region('US:FL').fiat('iso:USD').supported
    ).toBe(true)
    expect(
      store.is.direction('buy').region('*').fiat('iso:USD').supported
    ).toBe(true)
    // all rule -> false
    expect(
      store.is.direction('buy').region('US:TX').fiat('iso:USD').supported
    ).toBe(false)
    expect(
      store.is.direction('buy').region('US').fiat('iso:USD').supported
    ).toBe(false)

    // any rule -> true
    expect(
      store.is.direction('buy').region('UK').fiat('iso:GBP').supported
    ).toBe(true)
    expect(
      store.is.direction('buy').region('UK:JQ').fiat('iso:GBP').supported
    ).toBe(true)
    expect(
      store.is.direction('buy').region('CA').fiat('iso:CAD').supported
    ).toBe(true)
    expect(
      store.is.direction('buy').region('CA:QC').fiat('iso:CAD').supported
    ).toBe(true)
    // any rule -> false
    expect(
      store.is.direction('buy').region('UK').fiat('iso:USD').supported
    ).toBe(false)
    expect(
      store.is.direction('buy').region('UK:JQ').fiat('iso:USD').supported
    ).toBe(false)
    expect(
      store.is.direction('buy').region('CA:QC').fiat('iso:USD').supported
    ).toBe(false)
    expect(
      store.is.direction('buy').region('CA').fiat('iso:USD').supported
    ).toBe(false)

    // match-all queries:
    expect(
      store.is.direction('buy').region('*').fiat('iso:CAD').supported
    ).toBe(true)
    expect(
      store.is.direction('buy').region('*').fiat('iso:CAD').supported
    ).toBe(true)
    ...
    expect(store.is.direction('buy').region('UK').fiat('*').supported).toBe(
      true
    )
    expect(store.is.direction('*').region('UK').fiat('iso:GBP').supported).toBe(
      true
    )
    expect(store.is.direction('*').region('UK').fiat('iso:USD').supported).toBe(
      false
    )
  })
  ...
  it('queries across branches', () => {
    ...
    expect(store.is.direction('*').region('IT').fiat('iso:CHF').supported).toBe(
      true
    )
    expect(store.is.direction('*').region('FR').fiat('iso:CHF').supported).toBe(
      true
    )
    expect(store.is.direction('*').region('FR').fiat('iso:EUR').supported).toBe(
      true
    )
    expect(
      store.is.direction('*').region('FR').fiat('iso:EUR').payment('sepa')
        .supported
    ).toBe(true)
    expect(
      store.is
    ...
    expect(
      store.is.direction('*').region('US').fiat('iso:EUR').payment('sepa')
        .supported
    ).toBe(false)
    expect(store.is.direction('*').region('US').fiat('iso:EUR').supported).toBe(
      false
    )
    ...
    expect(
      store.is.direction('buy').region('FR').fiat('iso:EUR').payment('sepa')
        .supported
    ).toBe(false)
    expect(
      store.is.direction('buy').region('FR').fiat('iso:EUR').supported
    ).toBe(false)
    ...
    expect(
      store.is.direction('*').region('*').fiat('iso:USD').payment('sepa')
        .supported
    ).toBe(false)
    expect(store.is.direction('*').region('*').fiat('iso:USD').supported).toBe(
      false
    )
  ...
  describe('getFiatProviderAssetMap', () => {
    ...
    function makeTestCase(fixture: ProviderSupportObject): Tester {
      ...
      const tester: Tester = params => {
        ...
        const crypto: string[] = Object.keys(result.crypto).flatMap(
          pluginId => {
            return result.crypto[pluginId].map(
              token => `${pluginId}:${token.tokenId}`
            )
          }
        )
    ...
    it('will match query-all with match-all node', () => {
      ...
      expect(
        store.is.direction('*').region('*').fiat('*').payment('sepa').supported
      ).toBe(true)
    })

    it('will return crypto and fiat for a given query', () => {
      expect(
        test({ direction: 'sell', region: 'IT', payment: 'sepa' })
      ).toStrictEqual({
        crypto: [
          'ethereum:null',
    ...
    it('will return empty results for wrong direction', () => {
      expect(
        test({ direction: 'buy', region: 'IT', payment: 'sepa' })
      ).toStrictEqual({
        crypto: [],
        fiat: []
      })
    ...
    it('will return empty results for wrong region', () => {
      expect(
        test({ direction: 'sell', region: 'US', payment: 'sepa' })
      ).toStrictEqual({
        crypto: [],
        fiat: []
      })
    ...
    it('will return empty results for wrong payment', () => {
      expect(
        test({ direction: 'sell', region: 'IT', payment: 'ach' })
      ).toStrictEqual({
        crypto: [],
        fiat: []
      })
  ...
  describe('queryNodes', () => {
    ...
    const allToMap = (arr: InternalTreeRecord[]): InternalTree[] =>
      arr.map(toMap)
...
function makeBityStoreFixture(): ProviderSupportStore {
  ...
  store.add
    ...
    .fiat('*')
    .payment('sepa')
    .crypto('ethereum:a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48')
  store.add
    .direction('*')
    .region('*')
    .fiat('*')
    .payment('sepa')
    .crypto('ethereum:dac17f958d2ee523a2206206994597c13d831ec7')
