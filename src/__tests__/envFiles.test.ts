...
describe('makeEnvFromFiles', () => {
  ...
  it('passes plain-string swap secrets through without transforming them', () => {
    // Changelly's apiKey used to be XOR-masked in env.json; with remote getKeys
    // it is a plain string in keys.json, so the merge must leave it alone.
    const config = { swapPlugins: { changelly: { partnerId: 'edge' } } }
    const keys = { pluginApiKeys: { changelly: { apiKey: 'hunter2' } } }
    const env = makeEnvFromFiles(config, keys)
    expect((env.swapPlugins as any).changelly).toEqual({
      partnerId: 'edge',
      apiKey: 'hunter2'
    })
  })
...
// A legacy-shaped fixture covering every classification branch `splitEnv` has:
// booleans, mixed secret/non-secret plugin objects, bare-string provider keys,
// the two migrated `*_INIT` fields, a dropped `*_INIT`, and both flavours of
// top-level field. The golden suite runs against the developer's real env.json
// when one is present, and against this otherwise, so CI always exercises it.
const FIXTURE_ENV = {
  APP_CONFIG: 'edge',
  DEBUG_CORE: false,
  EDGE_API_KEY: 'edge-api-key',
  SENTRY_DSN_URL: 'https://sentry.example',
  BITCOIN_INIT: { nowNodesApiKey: 'nownodes' },
  ETHEREUM_INIT: {
    evmScanApiKey: ['scan1', 'scan2'],
    alchemyApiKey: 'alchemy',
    quiknodeApiKey: 'quiknode'
  },
  FIO_INIT: { fioRegApiToken: 'fio-token', freeAddressRef: 'edge' },
  DASH_INIT: false,
  THORCHAIN_INIT: { ninerealmsClientId: 'edge', affiliateFeeBasis: '50' },
  WYRE_CLIENT_INIT: { secret: 'dropped' },
  PLUGIN_API_KEYS: {
    moonpay: 'bare-string-key',
    banxa: { partnerUrl: 'https://banxa.example', apiKey: 'banxa-key' }
  },
  RAMP_PLUGIN_INITS: {
    banxa: { partnerUrl: 'https://ramp.banxa.example', apiKey: 'ramp-key' }
  },
  POSTHOG_INIT: { apiKey: 'ph-key', apiHost: 'https://ph.example' },
  WALLET_CONNECT_INIT: { projectId: 'wc-project' }
}

describe('golden equivalence with legacy env.json', () => {
  const envJsonPath = path.join(__dirname, '../../env.json')
  const legacyEnv: Record<string, any> = fs.existsSync(envJsonPath)
  ...
  it('round-trips every currency plugin init', () => {
  ...
  it('round-trips every swap plugin init', () => {
  ...
  it('round-trips PLUGIN_API_KEYS providers', () => {
  ...
  it('moves POSTHOG_INIT into pluginApiKeys.posthog', () => {
  ...
  it('moves WALLET_CONNECT_INIT into pluginApiKeys.walletconnect', () => {
