...
describe('processRevolutConfig', function () {
  it('should add supported assets', function () {
    ...
    expect(supportedAssets.is.direction('buy').region('GB').supported).toBe(true)
    expect(supportedAssets.is.direction('buy').region('GB').supported).toBe(true)
    expect(supportedAssets.is.direction('sell').region('GB:FOO').supported).toBe(true)
    expect(supportedAssets.is.direction('sell').region('GB:FOO').supported).toBe(true)

    // Fiat support
    expect(supportedAssets.is.direction('buy').region('GB').fiat('iso:GBP').supported).toBe(true)
    expect(supportedAssets.is.direction('sell').region('GB').fiat('iso:GBP').supported).toBe(true)

    // Crypto support
    expect(supportedAssets.is.direction('buy').region('GB').fiat('iso:GBP').payment('revolut').crypto('bitcoin:null').supported).toBe(true)
    expect(supportedAssets.is.direction('sell').region('GB').fiat('iso:GBP').payment('revolut').crypto('bitcoin:null').supported).toBe(true)
    expect(supportedAssets.is.direction('buy').region('GB').fiat('iso:GBP').payment('revolut').crypto('ethereum:null').supported).toBe(true)
    expect(supportedAssets.is.direction('sell').region('GB').fiat('iso:GBP').payment('revolut').crypto('ethereum:null').supported).toBe(true)

    // Token support
    expect(
      supportedAssets.is.direction('buy').region('GB').fiat('iso:GBP').payment('revolut').crypto('ethereum:a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48').supported
    ).toBe(true)
    expect(
      supportedAssets.is.direction('sell').region('GB').fiat('iso:GBP').payment('revolut').crypto('ethereum:a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48').supported
    ).toBe(true)

    // Wildcards
    expect(supportedAssets.is.direction('*').region('*').fiat('*').payment('*').crypto('bitcoin:null').supported).toBe(true)
    expect(supportedAssets.is.direction('*').region('*').fiat('*').payment('*').crypto('ethereum:null').supported).toBe(true)
    expect(supportedAssets.is.direction('*').region('*').fiat('*').payment('*').crypto('ethereum:a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48').supported).toBe(
      true
    )
  })
