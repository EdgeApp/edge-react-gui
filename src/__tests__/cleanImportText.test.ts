...
describe('cleanupImportText', function () {
  test('handles good inputs', function () {
    expect(cleanupImportText('super smol test mnemonic seed phrase')).toBe(
      'super smol test mnemonic seed phrase'
    )
    expect(cleanupImportText('14HyjvA79tpVwqdxvHZe4emcuskYSKnTCU')).toBe(
      '14HyjvA79tpVwqdxvHZe4emcuskYSKnTCU'
    )
  })
  ...
  test('trims whitespace characters', function () {
    ...
    expect(cleanupImportText('\r\t\f14HyjvA79tpVwqdxvHZe4emcuskYSKnTCU ')).toBe(
      '14HyjvA79tpVwqdxvHZe4emcuskYSKnTCU'
    )
  })
  ...
  test('removes extra spaces from phrases', function () {
    expect(
      cleanupImportText('super              smol   test mnemonic seed phrase  ')
    ).toBe('super smol test mnemonic seed phrase')
  })
