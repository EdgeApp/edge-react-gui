...
const makeWallet = (
  ...
  return {
    id: 'wallet-1',
    syncStatus: { totalRatio: syncRatio },
    currencyInfo: {
      pluginId: 'bitcoin',
      currencyCode: 'BTC',
...
interface StateOptions {
  balance?: string
  /** Active wallets the account has yet to load, e.g. mid-login. */
  pendingWallets?: number
  /** Sync progress of the funded wallet, below 1 while it is still syncing. */
  syncRatio?: number
  hasRate?: boolean
  recoveryKey?: string
...
const makeState = (opts: StateOptions = {}): RootState => {
  ...
  const currencyWallets = {
    'wallet-1': makeWallet(balance, tokenBalance, syncRatio),
  ...
  const account = {
    ...
    currencyWallets,
    activeWalletIds: [
      ...Object.keys(currencyWallets),
      ...Array.from({ length: pendingWallets }, (_, i) => `pending-${i}`)
    ]
...
describe('checkPasswordRecovery', () => {
  ...
  it('waits for the account to finish loading its wallets', async () => {
    // Pricing now would credit only $20 and leave the wallet still loading to
    // cross $200 later, which is a second modal for one login:
    const { levels, modalShown } = await run(
      makeState({ balance: '500000', pendingWallets: 1 })
    )
    expect(levels).toEqual([])
    expect(modalShown).toBe(false)
  })

  it('waits for a wallet that is still syncing', async () => {
    // The disk-cached balance is partial: transactions that arrived while the
    // app was closed replay during the sync and would cross the next level:
    const { levels, modalShown } = await run(
      makeState({ balance: '500000', syncRatio: 0.4 })
    )
    expect(levels).toEqual([])
    expect(modalShown).toBe(false)
  })

  it('stops waiting for a sync that never completes', async () => {
    const state = makeState({ balance: '500000', syncRatio: 0.4 })
  ...
  it('stops waiting for wallets that never load', async () => {
    const state = makeState({ balance: '500000', pendingWallets: 1 })
    const opening = await run(state)
    expect(opening.levels).toEqual([])
