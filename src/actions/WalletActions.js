...
import {
  getCurrencyInfos,
  makeCreateWalletType
} from '../util/CurrencyInfoHelpers.js'
import * as UTILS from '../util/utils'
import { addTokenAsync } from './AddTokenActions.js'

export const refreshReceiveAddressRequest =
  (walletId: string) => (dispatch: Dispatch, getState: GetState) => {
...
export const selectWallet =
  (walletId: string, currencyCode: string, from?: string) =>
...
export const selectEOSWallet =
  ...
  (dispatch: Dispatch, getState: GetState) => {
    ...
    if (
      walletId !== currentWalletId ||
      currencyCode !== currentWalletCurrencyCode ||
      from === Constants.WALLET_LIST_SCENE
    ) {
      ...
      if (publicAddress) {
        ...
        const fiatTypeIndex = supportedFiats.findIndex(
          fiatType => fiatType.value === guiWallet.fiatCurrencyCode
        )
        ...
        const currencyInfo = currencyInfos.find(
          info => info.currencyCode === currencyCode
        )
        ...
        Actions[Constants.CREATE_WALLET_ACCOUNT_SETUP](
          createWalletAccountSetupSceneProps
        )
        Airship.show(bridge => (
          <ButtonsModal
            bridge={bridge}
...
export const setWalletEnabledTokens =
  (walletId: string, enabledTokens: string[], disabledTokens: string[]) =>
  (dispatch: Dispatch, getState: GetState) => {
...
export const getEnabledTokens =
  (walletId: string) => async (dispatch: Dispatch, getState: GetState) => {
    // get a snapshot of the state
    const state = getState()
    const { currencyWallets } = state.core.account
...
export const assembleCustomToken = (
  currencyName: string,
  currencyCode: string,
...
export const editCustomToken = (
  ...
  return (dispatch: Dispatch, getState: GetState) => {
    ...
    const indexInAllTokens = _.findIndex(
      allTokens,
      token => token.currencyCode === currencyCode
    )
    const tokenObj = assembleCustomToken(
      currencyName,
      currencyCode,
      contractAddress,
      denomination,
    ...
    if (indexInAllTokens >= 0) {
      ...
      addTokenAsync(
        ...
        .then(addedTokenData => {
          deleteCustomTokenAsync(walletId, oldCurrencyCode, getState).then(
            coreWalletsToUpdate => {
...
export async function deleteCustomTokenAsync(
  walletId: string,
  currencyCode: string,
  getState: GetState
) {
  ...
  const syncedCustomTokens: CustomTokenInfo[] = [
    ...receivedSyncSettings.customTokens
  ]
  const indexOfSyncedToken: number = _.findIndex(
    syncedCustomTokens,
...
export const deleteCustomToken =
  (walletId: string, currencyCode: string) =>
  (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
    ...
    getSyncedSettings(account)
      .then(settings => {
        ...
        const indexOfTokenOnLocal = _.findIndex(
          customTokensOnLocal,
          item => item.currencyCode === currencyCode
        )
      ...
      .then(() => {
        coreWalletsToUpdate.forEach(wallet => {
          ...
          const newEnabledTokens = _.difference(
            guiWallets[wallet.id].enabledTokens,
            [currencyCode]
          )
          dispatch({
            type: 'UPDATE_WALLET_ENABLED_TOKENS',
...
export const updateWalletLoadingProgress =
  (walletId: string, newWalletProgress: number) =>
  (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const currentWalletProgress =
      state.ui.wallets.walletLoadingProgress[walletId]
...
export const updateMostRecentWalletsSelected =
  (walletId: string, currencyCode: string) =>
  (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
...
export const removeMostRecentWallet =
  (walletId: string, currencyCode: string) =>
  (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
    const { mostRecentWallets } = state.ui.settings
    const currentMostRecentWallets = mostRecentWallets.filter(
      wallet => wallet.id !== walletId || wallet.currencyCode !== currencyCode
    )
    setMostRecentWalletsSelected(account, currentMostRecentWallets)
      .then(() => {
        dispatch({
...
export const checkEnabledTokensArray =
  (walletId: string, newEnabledTokens: string[]) =>
  (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    ...
    oldEnabledTokens.forEach(oldToken => {
      const checkedToken = newEnabledTokens.find(
