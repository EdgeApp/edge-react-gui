...
export const updateOneSetting =
  (setting: Object) => (dispatch: Dispatch, getState: GetState) => {
...
export const setPINModeRequest =
  (pinMode: boolean) => (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
...
export const setAutoLogoutTimeInSecondsRequest =
  ...
  (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
    ACCOUNT_SETTINGS.setAutoLogoutTimeInSecondsRequest(
      account,
...
export const setDefaultFiatRequest =
  (defaultFiat: string) => (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
...
export const setMerchantModeRequest =
  (merchantMode: boolean) => (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
...
export const setPreferredSwapPluginId =
  (pluginId: string | void) => (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
...
export const setBluetoothModeRequest =
  (bluetoothMode: boolean) => (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
...
export const setDenominationKeyRequest =
  (currencyCode: string, denominationKey: string) =>
  (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core

    return ACCOUNT_SETTINGS.setDenominationKeyRequest(
      account,
      currencyCode,
      denominationKey
    )
...
export const updateTouchIdEnabled =
  (isTouchEnabled: boolean, account: EdgeAccount) =>
...
export const showReEnableOtpModal =
  () => async (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
...
export const enableCustomNodes =
  (currencyCode: string) => async (dispatch: Dispatch, getState: GetState) => {
    ...
    try {
      await currencyPlugin.changeUserSettings({
        ...currencyPlugin.userSettings,
...
export const disableCustomNodes =
  (currencyCode: string) => async (dispatch: Dispatch, getState: GetState) => {
    const state: RootState = getState()
    const { account } = state.core
    const currencyPluginName = CURRENCY_PLUGIN_NAMES[currencyCode]
...
export const saveCustomNodesList =
  (currencyCode: string, nodesList: string[]) =>
  async (dispatch: Dispatch, getState: GetState) => {
    const state: RootState = getState()
    const { account } = state.core
    const currencyPluginName = CURRENCY_PLUGIN_NAMES[currencyCode]
...
export const showUnlockSettingsModal =
  () => async (dispatch: Dispatch, getState: GetState) => {
    try {
      const input = {
...
export const submitLogs = (notes: string) => async (dispatch: Dispatch) => {
  try {
    await showActivity(
      s.strings.settings_modal_send_logs_loading,
...
export const showRestoreWalletsModal =
  () => async (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
    const response = await Airship.show(bridge => (
      <ButtonsModal
        ...
        buttons={{
          ...
          cancel: {
            label: s.strings.restore_wallets_modal_cancel,
            type: 'secondary'
          }
        }}
      />
    ...
    if (response === 'confirm') {
      const restoreKeys = account.allKeys.filter(
        key => key.archived || key.deleted
      )
      await Promise.all(
        restoreKeys
          .map(key => key.id)
...
export const setDeveloperModeOn =
  (developerModeOn: boolean) => (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
