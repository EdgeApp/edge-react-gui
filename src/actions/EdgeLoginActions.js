...
export const loginWithEdge =
  (lobbyId: string) => (dispatch: Dispatch, getState: GetState) => {
...
export const lobbyLogin =
  () => async (dispatch: Dispatch, getState: GetState) => {
    ...
    try {
      ...
      setTimeout(() => {
        Alert.alert(
          s.strings.send_scan_edge_login_success_title,
          s.strings.send_scan_edge_login_success_message
        )
      }, 750)
    } catch (e) {
      dispatch({ type: 'EDGE_LOBBY_ACCEPT_FAILED' })
      e.message = e.message.includes('Could not reach')
        ? s.strings.edge_login_fail_message
        : e.message
