...
export function checkPasswordRecovery(
  ...
  return (dispatch, getState) => {
    ...
    if (ratesSettling && !isBalanceSettled(state)) return
...
/**
 * True when the account's balance is worth pricing: every active wallet has
 * loaded and synced, and every funded one has a rate.
 *
 * Each of those three arrives separately during login, and each one re-runs
 * this check. Pricing early credits a milestone from a partial balance, so the
 * piece that lands next crosses the following one and shows a second,
 * identical reminder: a wallet still loading, or one still replaying the
 * transactions that arrived while the app was closed, is balance this account
 * already holds. The check stays inside the bounded grace window, so an
 * account that never finishes is still reminded rather than suppressed for the
 * session.
 */
function isBalanceSettled(state: RootState): boolean {
  const { activeWalletIds, currencyWallets } = state.core.account
  if (Object.keys(currencyWallets).length < activeWalletIds.length) return false

  for (const walletId of Object.keys(currencyWallets)) {
    if (currencyWallets[walletId].syncStatus.totalRatio < 1) return false
  }
  return hasRatesForFundedWallets(state)
}
