...
export const setPreferredSwapPluginId = (pluginId: string | undefined) => (dispatch: Dispatch, getState: GetState) => {
  const state = getState()
  const { account } = state.core
...
export const setDenominationKeyRequest =
  (pluginId: string, currencyCode: string, denomination: EdgeDenomination) => async (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
    const { account } = state.core
