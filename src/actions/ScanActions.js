...
export const doRequestAddress = (
  dispatch: Dispatch,
  edgeWallet: EdgeCurrencyWallet,
  guiWallet: GuiWallet,
  link: ReturnAddressLink
) => {
  ...
  if (currencyName !== edgeWallet.currencyInfo.pluginId) {
    ...
    const body = sprintf(
      s.strings.currency_mismatch_popup_body,
      currencyName,
      currencyName
    )
    setTimeout(
    ...
    const bodyString =
      sprintf(
        s.strings.request_crypto_address_modal_body,
        sourceName,
        currencyName
      ) + '\n\n'
    ...
    setTimeout(() => {
      Airship.show(bridge => (
        <ButtonsModal
          ...
          buttons={{
            confirm: {
              label: s.strings.request_crypto_address_modal_send_address_button
            },
...
export const parseScannedUri =
  (data: string, customErrorTitle?: string, customErrorDescription?: string) =>
...
export const qrCodeScanned =
  (data: string) => (dispatch: Dispatch, getState: GetState) => {
    const state = getState()
...
export const showLegacyAddressModal =
  () => async (dispatch: Dispatch, getState: GetState) => {
    const response = await Airship.show(bridge => (
      <ButtonsModal
        ...
        buttons={{
          ...
          cancel: {
            label: s.strings.legacy_address_modal_cancel,
            type: 'secondary'
          }
        }}
      />
...
export const privateKeyModalActivated =
  () => async (dispatch: Dispatch, getState: GetState) => {
    const firstResponse = await Airship.show(bridge => (
      <ButtonsModal
        ...
        buttons={{
          ...
          cancel: {
            label: s.strings.private_key_modal_cancel,
            type: 'secondary'
          }
        }}
      />
      ...
      edgeWallet.sweepPrivateKeys(spendInfo).then(
        (unsignedTx: EdgeTransaction) => {
          edgeWallet
            ...
            .then(() =>
              dispatch({ type: 'PRIVATE_KEY_MODAL/SWEEP_PRIVATE_KEY_SUCCESS' })
            )
        },
        (error: Error) => {
          console.log(error)
...
export const checkAndShowGetCryptoModal =
  ...
  async (dispatch: Dispatch, getState: GetState) => {
    try {
      ...
      const wallet =
        wallets[selectedWalletId || state.ui.wallets.selectedWalletId]
      // check if balance is zero
      const balance = wallet.nativeBalances[currencyCode]
      if (balance !== '0' || shownWalletGetCryptoModals.includes(wallet.id))
      ...
      if (displayBuyCrypto) {
        const messageSyntax = sprintf(
          s.strings.buy_crypto_modal_message,
          currencyCode,
          currencyCode,
          currencyCode
        )
        ...
        const messageSyntax = sprintf(
          s.strings.exchange_crypto_modal_message,
          currencyCode,
          currencyCode,
          currencyCode
        )
