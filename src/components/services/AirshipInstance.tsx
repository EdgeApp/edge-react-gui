 ...
 */
export async function showDevErrorAsync(
  error: unknown,
  options: ShowErrorWarningOptions = {}
): Promise<void> {
  ...
  if (__DEV__ || appVersion === '99.99.99' || appVersion.includes('-d')) {
    ...
    await Airship.show(bridge => (
      <AlertDropdown bridge={bridge} persistent message={translatedMessage} />
    ))
  } else {
...
export function showWarning(
  ...
  const { trackError: doTrackError = true, tag } = options
  const translatedError = tag
    ? `Tag: ${tag}. ` + translateError(error)
    : translateError(error)
  if (doTrackError) {
    trackError(error, tag)
  }
  console.log(
    yellowText('Showing warning drop-down alert: ' + makeErrorLog(error))
  )
  Airship.show(bridge => (
