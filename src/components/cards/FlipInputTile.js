...
const FlipInputTileComponent = (props: Props) => {
  ...
  const isoFiatCurrencyCode = useWatchWallet(wallet, 'fiatCurrencyCode')
  const walletId = useWatchWallet(wallet, 'id')
  const currencyInfo = useWatchWallet(wallet, 'currencyInfo')
  const { pluginId } = currencyInfo
  const { allTokens } = useWatchWallet(wallet, 'currencyConfig')
