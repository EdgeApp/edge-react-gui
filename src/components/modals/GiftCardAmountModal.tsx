  ...
  const renderAmountRow: ListRenderItem<GiftCardAmountItem> = React.useCallback(
    ({ item }) => {
      ...
      if (item.isMinimum === true) {
        labelSuffix = lstrings.gift_card_minimum
      } else if (item.isMaximum === true) {
        labelSuffix = lstrings.gift_card_maximum
      }
      ...
      return (
        <EdgeTouchableOpacity style={styles.amountRow} onPress={handlePress}>
          ...
          <EdgeText style={styles.amountValue}>
            {`${fiatSymbol}${item.amount} ${item.currency}`}
          </EdgeText>
        </EdgeTouchableOpacity>
      )
    },
    [handleAmountSelect, styles, fiatSymbol]
  )
          ...
          <View style={styles.amountTextContainer}>
            ...
            {selectedItem.isMinimum === true ? (
              <EdgeText style={styles.amountLabel}>
                {lstrings.gift_card_minimum}
              </EdgeText>
            ) : selectedItem.isMaximum === true ? (
              <EdgeText style={styles.amountLabel}>
                {lstrings.gift_card_maximum}
              </EdgeText>
            ) : null}
          </View>
          <EdgeText style={styles.amountValue}>
            {`${fiatSymbol}${selectedAmount} ${currency}`}
          </EdgeText>
        </EdgeTouchableOpacity>
      </View>
