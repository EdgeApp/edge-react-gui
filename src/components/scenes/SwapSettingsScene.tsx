...
import {
  setPreferredSwapPluginId,
  setPreferredSwapPluginType
...
export class SwapSettings extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    const cexIds = exchangeIds.filter(
      id => swapConfigs[id].swapInfo.isDex !== true
    )
    const dexIds = exchangeIds.filter(
      id => swapConfigs[id].swapInfo.isDex === true
    )
    this.sortedCexIds = cexIds.sort((a, b) =>
      swapConfigs[a].swapInfo.displayName.localeCompare(
        swapConfigs[b].swapInfo.displayName
      )
    )
    this.sortedDexIds = dexIds.sort((a, b) =>
      swapConfigs[a].swapInfo.displayName.localeCompare(
        swapConfigs[b].swapInfo.displayName
      )
    )
  ...
  componentDidMount(): void {
    ...
    this.swapConfigUnsubscribeFns = []
    for (const pluginId of Object.keys(swapConfigs)) {
      const unsubscribe = swapConfigs[pluginId].watch(
        'enabled',
        pluginEnabled => {
          this.setState({
  ...
  handlePreferredModal = async () => {
    ...
    if (settingsPreferredSwapType === 'DEX') {
      ...
      selected =
        swapConfigs[selectedPluginId] != null
          ? swapConfigs[selectedPluginId].swapInfo.displayName
          : lstrings.swap_preferred_cheapest
    }
    ...
    const exchangeItems = [...dexItems, ...cexItems]
      ...
      .filter(
        pluginId => swapConfigs[pluginId].enabled && pluginId !== 'transfer'
      )
      .map(pluginId => ({
    ...
    if (result === preferDex.name) {
      ...
      changePreferredSwapPlugin(
        Object.keys(swapConfigs).find(
          pluginId => swapConfigs[pluginId].swapInfo.displayName === result
        )
      )
    }
  }
    ...
    return (
      <SceneWrapper>
        ...
        >
          ...
          <SettingsHeaderRow
            label={lstrings.swap_options_header_decentralized}
          />
  ...
  renderPreferredArea() {
    ...
    if (swapSource.type === 'promotion') {
      return (
        <>
          <SettingsSubHeader
            label={lstrings.swap_preferred_promo_instructions}
          />
          <SettingsTappableRow
            action="delete"
            label={label}
