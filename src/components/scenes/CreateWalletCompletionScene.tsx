...
const CreateWalletCompletionComponent = (props: Props) => {
  ...
  useAsyncEffect(
    async () => {
      ...
      setWallets(walletResults.filter(result => result.ok).map(result => result.result))
  ...
  const handleMigrate = useHandler(() => {
    ...
    const migrateWalletList: MigrateWalletItem[] = newWalletItems.map(createWallet => {
      ...
      const { currencyCode, key, pluginId, tokenId, walletType } = createWallet
      ...
      {({ insetStyle, undoInsetStyle }) => (
        <View style={{ ...undoInsetStyle, marginTop: 0 }}>
          ...
          <SceneButtons
            ...
            secondary={{ label: lstrings.migrate_label, disabled: !done, onPress: handleMigrate }}
          />
        </View>
      )}
    </SceneWrapper>
