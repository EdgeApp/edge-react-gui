...
export const FioConnectWalletConfirmComponent = (props: FioConnectWalletConfirmProps) => {
  ...
  const updateConnectedWallets = useHandler((currentFioAddress: string, currentCcWalletMap: CcWalletMap) => {
    dispatch({
  ...
  const handleCheck = useHandler(() => {
    setAcknowledge(!acknowledge)
  })

  const renderWalletLine = useHandler((wallet: FioConnectionWalletItem) => {
  ...
  const confirm = useHandler(async () => {
    if (!isConnected) {
  ...
  return (
    <SceneWrapper scroll>
      ...
      <View style={styles.container}>
        ...
        <Radio value={acknowledge} onPress={handleCheck} marginRem={[2, 2, 0]}>
