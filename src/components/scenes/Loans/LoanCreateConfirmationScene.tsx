...
export const LoanCreateConfirmationScene = (props: Props) => {
  ...
  const { currencyCode: borrowEngineCurrencyCode } =
    borrowEngineWallet.currencyInfo
  ...
  const existingLoanAccount = useSelector(state =>
    selectLoanAccount(state, borrowEngineWallet.id)
  )
  const [loanAccount, loanAccountError] = useAsyncValue(
    async () =>
      existingLoanAccount ??
      (await makeLoanAccount(borrowPlugin, borrowEngine.currencyWallet)),
  ...
  const minFeeSwapAmount = useSelector(state => {
    ...
    if (polygonPrice === '0') return '0'
    return borrowPluginCurrencyPluginId === 'polygon' && isCrossChainSrc
      ? truncateDecimals(mul('5.05', div('1', polygonPrice)), 0)
      : '0'
  })

  const [[actionProgram, networkFeeMap = {}] = [], actionProgramError] =
    useAsyncValue(async () => {
      ...
      const mainNetworkFeeAmount =
        mainNetworkFee != null
          ? mul(
              mainNetworkFee.nativeAmount,
              FEE_VOLATILITY_MULTIPLIER[borrowPluginCurrencyPluginId]
            )
          : '0'
      ...
      if (
        ...
        const feeDeficitNativeAmount = sub(
          mainNetworkFeeAmount,
          borrowWalletNativeBalance
        )
  ...
  networkFeeAmount =
    isCrossChainSrc && gt(networkFeeAmount, borrowWalletNativeBalance)
  ...
  const handleSliderComplete = async (resetSlider: () => void) => {
    if (actionProgram != null && loanAccount != null) {
      try {
        ...
        if (!loanAccount.borrowEngine.isRunning)
          await loanAccount.borrowEngine.startEngine()

        await dispatch(saveLoanAccount(loanAccount))
        await dispatch(
          runLoanActionProgram(loanAccount, actionProgram, 'loan-create')
        )
  ...
  if (loanAccountError != null)
    return (
      <Alert
        title={lstrings.error_unexpected_title}
        type="error"
        message={translateError(loanAccountError)}
      />
    )
  ...
  return loanAccount == null ? (
    ...
    >
      ...
      {actionProgramError != null ? (
        <ErrorTile message={actionProgramError.message} />
      ) : null}
      <NetworkFeeTile
        wallet={borrowEngineWallet}
        nativeAmount={networkFeeAmount}
      />
      {isFeesExceedCollateral ? (
        <ErrorTile message={lstrings.loan_amount_fees_exceeds_collateral} />
      ) : null}
    </FormScene>
  )
}
