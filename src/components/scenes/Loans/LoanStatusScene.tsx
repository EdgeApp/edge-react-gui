...
export const LoanStatusSceneComponent = (props: Props) => {
  ...
  const [displayInfo] = useAsyncValue<ActionDisplayInfo>(async () => {
    ...
    const displayInfo = await getActionProgramDisplayInfo(
      account,
      program,
      state
    )

    if (displayInfo != null && displayInfo.status instanceof Error)
      return displayInfo
  ...
  const completeMessage =
    displayInfo != null ? displayInfo.completeMessage : undefined
  ...
  const handleCancelPress = useHandler(async () => {
    const approve = await Airship.show<boolean>(bridge => (
      <ConfirmContinueModal
        ...
        body={sprintf(
          lstrings.loan_status_cancel_txs_modal_msg,
          config.appName
        )}
        isSkippable
      />
  ...
  const handleInfoIconPress = useUrlHandler(
    sprintf(AAVE_SUPPORT_ARTICLE_URL_1S, 'loan-status')
  )

  const handleDonePress = useHandler(() => {
    if (loanAccountId != null)
      navigation.navigate('loanDetails', { loanAccountId })
      ...
      {displayInfo == null ? (
        ...
        <View style={styles.container}>
          ...
          <View style={styles.footerContainer}>
            {isProgramDone ? (
              <>
                <EdgeText style={styles.textCompleteTitle}>
                  {completeTitle}
                </EdgeText>
                <EdgeText style={styles.textCompleteInfo}>
                  {completeBody}
                </EdgeText>
              </>
            ) : null}
          ...
          {isProgramDone ? (
            <MainButton
              ...
              marginRem={buttonMargin}
            />
          ) : (
            <MainButton
              label={lstrings.loan_status_cancel_txs}
              type="secondary"
