...
import { showRestoreWalletsModal } from '../../actions/SettingsActions'
...
export function WalletRestoreScene(props: Props) {
  ...
  const restoreWalletInfos = allKeys.filter(key => key.archived || key.deleted).reverse()
  ...
  const handlePressDone = useHandler(async () => {
    await dispatch(showRestoreWalletsModal(navigation, selectedWalletInfos))
  })
...
const RestoreList = (props: {
  ...
  const filteredWalletInfos = React.useMemo(() => {
    ...
    for (const restoreWalletInfo of restoreWalletInfos) {
      ...
      const pluginId = type.replace('wallet:', '')
      const { currencyCode, displayName } = currencyConfigMap[pluginId]?.currencyInfo ?? {}
    ...
    return out
  }, [currencyConfigMap, restoreWalletInfos, searchText])
