...
> {
  ...
  render() {
    ...
    const walletName = `${
      this.props.sourceWallet.name || s.strings.string_no_wallet_name
    ...
    return (
      <SceneWrapper background="theme">
        <ScrollView>
          ...
          />
          <SettingsRow
            text={s.strings.export_transaction_this_month}
            onPress={this.setThisMonth}
          />
          <SettingsRow
            text={s.strings.export_transaction_last_month}
            onPress={this.setLastMonth}
          />
          <SettingsLabelRow
            text={s.strings.string_start}
            right={startDateString}
            onPress={this.handleStartDate}
          />
          <SettingsLabelRow
            text={s.strings.string_end}
            right={endDateString}
            onPress={this.handleEndDate}
          />
          <SettingsHeaderRow
            icon={
              <EntypoIcon name="export" color={theme.icon} size={iconSize} />
            }
            text={s.strings.export_transaction_export_type}
          />
          {Platform.OS === 'android'
            ? this.renderAndroidSwitches()
            : this.renderIosSwitches()}
          {disabledExport ? null : (
            <PrimaryButton
              label={s.strings.string_export}
              marginRem={1.5}
  ...
  renderAndroidSwitches(): React.Node {
    ...
    return (
      <>
        <SettingsRadioRow
          key="exportQbo"
          text={s.strings.export_transaction_quickbooks_qbo}
          value={isExportQbo}
          onPress={this.handleAndroidToggle}
        />
        <SettingsRadioRow
          key="exportCsv"
          text={s.strings.export_transaction_csv}
          value={isExportCsv}
          onPress={this.handleAndroidToggle}
        />
      </>
  ...
  pickFileName() {
    ...
    const walletName =
      sourceWallet.name != null
        ? sourceWallet.name
        : s.strings.string_no_wallet_name
  ...
  async exportFiles(): Promise<void> {
    ...
    const csvFile = await showActivity(
      s.strings.export_transaction_loading,
      exportTransactionsToCSV(this.props.sourceWallet, transactionOptions)
    )
    ...
    if (isExportQbo) {
      const qboFile = await showActivity(
        s.strings.export_transaction_loading,
        exportTransactionsToQBO(sourceWallet, transactionOptions)
      )
      files.push({
        contents: qboFile,
...
export const TransactionsExportScene = connect(
  (state: RootState, ownProps: OwnProps): StateProps => {
    const denominationObject = getDisplayDenomination(
      state,
