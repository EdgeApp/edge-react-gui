...
} from '../../actions/TransactionDetailsActions.js'
...
export class TransactionDetailsComponent extends React.Component<Props, State> {
  constructor(props: Props) {
    ...
    const direction =
      parseInt(edgeTransaction.nativeAmount) >= 0 ? 'receive' : 'send'
    const category = this.initializeFormattedCategories(
    ...
    this.state = {
      ...
      payeeName:
        edgeTransaction.metadata && edgeTransaction.metadata.name
          ? edgeTransaction.metadata.name
          : '', // remove commenting once metaData in Redux
      notes:
        edgeTransaction.metadata && edgeTransaction.metadata.notes
          ? edgeTransaction.metadata.notes
          : '',
  ...
  initalizeAmountBalance = (metadata: ?EdgeMetadata) => {
    if (metadata && metadata.amountFiat) {
      ...
      return formatNumber(bns.toFixed(absoluteAmount, 2, 2), {
        noGrouping: true
      })
    }
  ...
  initializeFormattedCategories = (
    metadata: ?EdgeMetadata,
    direction: string
  ) => {
    const defaultCategory =
      direction === 'receive' ? categories.income.key : categories.expense.key
    if (metadata) {
      ...
      if (fullCategory && colonOccurrence) {
        const splittedFullCategory =
          UTILS.splitTransactionCategory(fullCategory)
  ...
  openPersonInput = () => {
    const personLabel =
      this.state.direction === 'receive'
        ? s.strings.transaction_details_payer
        : s.strings.transaction_details_payee
    Airship.show(bridge => (
  ...
  onChangeCategory = (category: string, subCategory: string) =>
  ...
  openAdvancedDetails = async () => {
    ...
    Airship.show(bridge => (
      <TransactionAdvanceDetails
        ...
        recipientAddress={
          this.props.edgeTransaction.spendTargets
            ? this.props.edgeTransaction.spendTargets[0].publicAddress
            : ''
        }
        url={
          currencyInfo
            ? sprintf(
                currencyInfo.transactionExplorer,
                this.props.edgeTransaction.txid
              )
            : undefined
        }
      />
  ...
  renderExchangeData = (symbolString: string) => {
    ...
    const { plugin, isEstimate, orderId, payoutAddress, refundAddress } =
      swapData
    const sourceAmount = UTILS.convertNativeToDisplay(
      walletDefaultDenomProps.multiplier
    )(spendTargets[0].nativeAmount)
    const destinationAmount = UTILS.convertNativeToDisplay(
    ...
    const createExchangeDataString = (newline: string = '\n') => {
      const destinationWalletName = destinationWallet
        ? destinationWallet.name
        : ''
      const uniqueIdentifier =
        spendTargets && spendTargets[0].uniqueIdentifier
          ? spendTargets[0].uniqueIdentifier
          : ''
      ...
      return `${s.strings.transaction_details_exchange_service}: ${
        plugin.displayName
      }${newline}${s.strings.transaction_details_exchange_order_id}: ${
        orderId || ''
      }${newline}${s.strings.transaction_details_exchange_source_wallet}: ${
        guiWallet.name
      }${newline}${
        s.strings.fragment_send_from_label
      }: ${sourceAmount} ${symbolString}${newline}${
        s.strings.string_to_capitalize
      }: ${destinationAmount} ${destinationCurrencyCode}${newline}${
        s.strings.transaction_details_exchange_destination_wallet
      }: ${destinationWalletName}${newline}${
        isEstimate ? s.strings.estimated_quote : s.strings.fixed_quote
      }${newline}${newline}${
        s.strings.transaction_details_exchange_exchange_address
      }:${newline}  ${exchangeAddresses}${newline}${
        s.strings.transaction_details_exchange_exchange_unique_id
      }:${newline}  ${uniqueIdentifier}${newline}${
        s.strings.transaction_details_exchange_payout_address
      }:${newline}  ${payoutAddress}${newline}${
        s.strings.transaction_details_exchange_refund_address
    ...
    const openEmail = () => {
      ...
      Mailer.mail(
        {
          subject: sprintf(
            s.strings.transaction_details_exchange_support_request,
    ...
    return (
      <>
        ...
        {swapData.orderUri && (
          <Tile
            type="touchable"
            title={s.strings.transaction_details_exchange_status_page}
            onPress={openUrl}
            body={swapData.orderUri}
          />
        )}
        {swapData.plugin.supportEmail && (
          <Tile
            type="touchable"
            title={s.strings.transaction_details_exchange_support}
            onPress={openEmail}
  ...
  onSaveTxDetails = () => {
    const { payeeName, notes, bizId, category, subCategory, amountFiat } =
      this.state
    const { edgeTransaction } = this.props
    let finalAmountFiat
    const fullCategory = category
      ? `${UTILS.capitalize(category)}:${subCategory}`
      : undefined
    ...
    if (isNaN(decimalAmountFiat)) {
      ...
      finalAmountFiat = edgeTransaction.metadata
        ? edgeTransaction.metadata.amountFiat
        : 0.0
    } else {
  ...
  getReceivedCryptoAmount(): FiatCryptoAmountUI {
    ...
    const convertedAmount = UTILS.convertNativeToDisplay(
      walletDefaultDenomProps.multiplier
    )(absoluteAmount)
    const currencyName = guiWallet.currencyNames[edgeTransaction.currencyCode]
    const symbolString =
      UTILS.isCryptoParentCurrency(guiWallet, edgeTransaction.currencyCode) &&
  ...
  getSentCryptoAmount(): FiatCryptoAmountUI {
    ...
    if (edgeTransaction.networkFee) {
      ...
      const convertedFee = UTILS.convertNativeToDisplay(
        walletDefaultDenomProps.multiplier
      ...
      const feeString = symbolString
        ? sprintf(
            s.strings.fragment_tx_detail_mining_fee_with_symbol,
            feeAbsolute
          )
        : sprintf(
            s.strings.fragment_tx_detail_mining_fee_with_denom,
            feeAbsolute,
            walletDefaultDenomProps.name
          )
      return {
  ...
  render() {
    ...
    const crypto: FiatCryptoAmountUI =
      direction === 'receive'
        ? this.getReceivedCryptoAmount()
        : this.getSentCryptoAmount()
    const fiatSymbol = UTILS.getFiatSymbol(guiWallet.fiatCurrencyCode)
    const fiatValue = UTILS.truncateDecimals(
      amountFiat.replace('-', ''),
      2,
      true
    )
    ...
    const personLabel =
      direction === 'receive'
        ? s.strings.transaction_details_sender
        : s.strings.transaction_details_recipient
    const personName =
      payeeName && payeeName !== '' ? this.state.payeeName : personLabel
    const personHeader = sprintf(
      s.strings.transaction_details_person_name,
      personLabel
    )
    ...
    const specialCurrencyInfo = edgeTransaction.wallet
      ? Constants.getSpecialCurrencyInfo(
          edgeTransaction.wallet.currencyInfo.currencyCode
        )
      : undefined
        ...
        <ScrollView>
          <View style={styles.tilesContainer}>
            ...
            <Tile
              type="static"
              title={sprintf(
                s.strings.transaction_details_crypto_amount,
                crypto.currencyName
              )}
              body={`${crypto.symbolString} ${crypto.amountString}${
            ...
            <Tile
              type="editable"
              title={sprintf(
                s.strings.transaction_details_amount_in_fiat,
                fiatCurrencyCode
              )}
            ...
            <Tile
              type="static"
              title={s.strings.transaction_details_amount_current_price}
            >
              <View style={styles.tileRow}>
                <EdgeText style={styles.tileTextBottom}>
                  {fiatSymbol + ' '}
                </EdgeText>
                <EdgeText style={styles.tileTextPrice}>
                  {currentFiat.amount}
                </EdgeText>
                <EdgeText
                  style={
                    parseFloat(currentFiat.difference) >= 0
                      ? styles.tileTextPriceChangeUp
                      : styles.tileTextPriceChangeDown
                  }
                >
                  {(parseFloat(currentFiat.difference) >= 0
                    ? currentFiat.percentage
                    : `- ${currentFiat.percentage}`) + '%'}
                </EdgeText>
              </View>
            </Tile>
            ...
            >
              <View style={styles.tileRow}>
                <View style={styles.tileCategory}>
                  <EdgeText style={styles.tileCategoryText}>
                    {categories[category].syntax}
                  </EdgeText>
                </View>
                <EdgeText style={styles.tileSubCategoryText}>
                  {subCategory}
                </EdgeText>
              </View>
            </Tile>
            {edgeTransaction.spendTargets && (
              <Tile
                type="copy"
                title={s.strings.transaction_details_recipient_addresses}
            ...
            <TouchableWithoutFeedback onPress={this.openAdvancedDetails}>
              <EdgeText style={styles.textAdvancedTransaction}>
                {s.strings.transaction_details_view_advanced_data}
              </EdgeText>
            </TouchableWithoutFeedback>
            <PrimaryButton
              onPress={this.onSaveTxDetails}
...
export const TransactionDetailsScene = connect(
  (state: RootState, ownProps: OwnProps): StateProps => {
    ...
    const wallet =
      state.ui.wallets.byId[walletId || state.ui.wallets.selectedWalletId]
    const contacts = state.contacts
    const subcategoriesList =
      state.ui.scenes.transactionDetails.subcategories.sort()
    ...
    const walletDefaultDenomProps: EdgeDenomination =
      UTILS.isCryptoParentCurrency(wallet, edgeTransaction.currencyCode)
        ? UTILS.getDenomination(wallet.currencyCode, settings, 'exchange')
        : UTILS.getDenomination(
    ...
    const nativeAmount =
      edgeTransaction && edgeTransaction.nativeAmount
        ? bns.abs(edgeTransaction.nativeAmount)
        : ''
    const cryptoAmount = convertNativeToExchangeRateDenomination(
      settings,
      currencyCode,
      nativeAmount
    )
    ...
    const destinationDenomination = swapData
      ? getDisplayDenomination(state, swapData.payoutCurrencyCode)
      : undefined
    const destinationWallet = swapData
      ? state.ui.wallets.byId[swapData.payoutWalletId]
      : undefined
