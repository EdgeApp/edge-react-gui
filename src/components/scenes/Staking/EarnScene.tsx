...
export const EarnScene = (props: Props) => {
  ...
  const [displayWallets, setDisplayWallets] = React.useState<EdgeCurrencyWallet[]>(wallets)
  ...
  React.useEffect(() => {
    ...
    if (isPortfolioSelected) {
      setDisplayWallets(positionWallets)
    } else {
      setDisplayWallets(wallets)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isPortfolioSelected, stakePolicyMap])

  const renderStakeItems = React.useCallback(
    (item: FlatListItem<EdgeCurrencyWallet>) => {
      const { item: wallet } = item
      if (stakePolicyMap == null) return <></>
    ...
    [navigation, stakePolicyMap, isPortfolioSelected]
  )
  ...
  return (
    ...
    <SceneWrapper scroll padding={theme.rem(0.5)}>
      ...
      <FlatList data={displayWallets} renderItem={renderStakeItems} keyExtractor={wallet => wallet.id} style={styles.list} />
