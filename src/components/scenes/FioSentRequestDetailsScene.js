...
type Props = FioSentRequestDetailsProps &
  FioSentRequestDetailsDispatchProps &
  NavigationProps &
  ThemeProps
...
class FioSentRequestDetailsComponent extends React.PureComponent<Props> {
  ...
  amountField = () => {
    ...
    return (
      <Tile type="static" title={s.strings.fio_request_amount} body={text} />
    )
  }
  ...
  render() {
    ...
    return (
      <SceneWrapper background="header">
        <SceneHeader
          title={s.strings.title_fio_sent_request_details}
          underline
        />
        {this.amountField()}
        <Tile
          type="static"
          title={s.strings.fio_request_sent_details_from}
          body={selectedFioSentRequest.payee_fio_address}
        />
        <Tile
          type="static"
          title={s.strings.fio_request_sent_details_to}
          body={selectedFioSentRequest.payer_fio_address}
        />
        {this.statusField(selectedFioSentRequest.status)}
        <Tile
          type="static"
          title={s.strings.fio_date_label}
          body={formatDate(new Date(selectedFioSentRequest.time_stamp), true)}
        />
        <Tile
          type="static"
          title={s.strings.unique_identifier_memo}
