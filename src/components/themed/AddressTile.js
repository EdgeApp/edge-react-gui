...
class AddressTileComponent extends React.PureComponent<Props, State> {
  ...
  paymentProtocolUriReceived = async ({
    paymentProtocolURL
  }: {
    ...
    try {
      ...
      const paymentProtocolInfo = await coreWallet.getPaymentProtocolInfo(
        paymentProtocolURL
      )
  ...
  onChangeAddress = async (address: string) => {
    ...
    try {
      ...
      // set address
      onChangeAddress(
        { fioAddress, isSendUsingFioAddress: !!fioAddress },
        parsedUri
      )
    } catch (e) {
      showError(
        `${s.strings.scan_invalid_address_error_title} ${s.strings.scan_invalid_address_error_description}`
      )
      this.setState({ loading: false })
    }
  }
  ...
  handleScan = () => {
    Airship.show(bridge => (
      <ScanModal bridge={bridge} title={s.strings.scan_qr_label} />
    ))
      .then((result: string) => {
        if (result) {
  ...
  render() {
    const { fioToAddress, recipientAddress, lockInputs, theme, title } =
      this.props
    const { loading } = this.state
    const styles = getStyles(theme)
    const copyMessage = this.state.clipboard
      ? `${s.strings.string_paste}: ${this.state.clipboard}`
      : null
    const tileType = loading
      ? 'loading'
      : !!recipientAddress && !lockInputs
      ? 'touchable'
      : 'static'
    return (
      <View>
          ...
          {!recipientAddress && (
            <View style={styles.buttonsContainer}>
              ...
              >
                ...
                />
                <EdgeText style={styles.buttonText}>
                  {s.strings.scan_as_in_scan_barcode}
                </EdgeText>
              </TouchableOpacity>
              {copyMessage && (
                <TouchableOpacity
                  style={styles.buttonContainer}
                  onPress={this.handlePasteFromClipboard}
                >
                  <FontAwesome5
                    name="clipboard"
                    size={theme.rem(2)}
...
const AddressTileConnector = connect((state: RootState): StateProps => {
  ...
  return {
    fioToAddress:
      guiMakeSpendInfo && guiMakeSpendInfo.fioAddress
        ? guiMakeSpendInfo.fioAddress
        : undefined,
    fioPlugin: account.currencyConfig
      ? account.currencyConfig[CURRENCY_PLUGIN_NAMES.FIO]
      : null,
