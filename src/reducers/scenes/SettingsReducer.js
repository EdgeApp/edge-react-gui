    ...
    case 'LOGIN': {
      ...
      for (const pluginId of Object.keys(account.currencyConfig)) {
        ...
        if (newState.denominationSettings[pluginId][currencyCode] == null) {
          newState.denominationSettings[pluginId][currencyCode] = getDefaultDenomination(currencyInfo, currencyCode)
        }
        for (const token of currencyInfo.metaTokens) {
          const tokenCode = token.currencyCode
          newState.denominationSettings[pluginId][tokenCode] = getDefaultDenomination(currencyInfo, tokenCode)
        }
      }
      return newState
    }
