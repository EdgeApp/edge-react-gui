...
import { EXIT, printError, printJson } from './client/output'
import {
  clearSessionFile,
  readSessionFile,
  writeSessionFile
  ...
  if (solveCaptcha && loginCommands.has(cmd.name)) {
    ...
    try {
      ...
      if (
        ...
        // Retry by appending/replacing — commands accept challenge via body
        // Re-run with env hint: stash challengeId on argv as JSON last arg for create/login
        // Simpler: call the REST endpoints directly for retry
        await retryLoginCommand(ctx, cmd.name, argv, challengeId)
      } else {
        throw error
      }
    }
...
async function retryLoginCommand(
  ...
  if (name === 'password-login') {
    ...
    const session = await ctx.client.post<{
      sessionId: string
      username?: string
    }>('/v1/login/password', { username, password, otp, challengeId })
    ctx.setSessionId(session.sessionId, session.username)
    printJson(session)
    return
  }
  if (name === 'account-create') {
    ...
    const session = await ctx.client.post<{
      sessionId: string
      username?: string
    }>('/v1/login/create', { username, password, pin, challengeId })
    ctx.setSessionId(session.sessionId, session.username)
    printJson(session)
    return
  }
  if (name === 'account-available') {
    ...
    const result = await ctx.client.get(
      `/v1/username-available?username=${encodeURIComponent(
        username
  ...
  if (name === 'pin-login') {
    ...
    const session = await ctx.client.post<{
      sessionId: string
      username?: string
    }>('/v1/login/pin', { username, pin, challengeId })
    ctx.setSessionId(session.sessionId, session.username)
    printJson(session)
    return
  }
