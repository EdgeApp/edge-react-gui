...
import { makePhazeGiftCardProvider, type PhazeGiftCardProvider } from '../plugins/gift-cards/phazeGiftCardProvider'
...
export function useGiftCardProvider(options: UseGiftCardProviderOptions): {
  ...
  const { account, apiKey, baseUrl = 'https://api.rewardsevolved.com/sandbox', publicKey } = options
  const [provider, setProvider] = React.useState<PhazeGiftCardProvider | null>(null)
