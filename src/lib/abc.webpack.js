...
/***/ function(module, exports, __webpack_require__) {
	...
	var aesjs = _interopDefault(__webpack_require__(5));
	var hashjs = _interopDefault(__webpack_require__(6));
	var scryptJs = _interopDefault(__webpack_require__(13));
	var baseX = _interopDefault(__webpack_require__(17));
	var elliptic = _interopDefault(__webpack_require__(18));
	...
	function random (bytes) {
	  ...
	  try {
	    ...
	    return __webpack_require__(39).randomBytes(bytes)
	  }
	}
	...
	function serialize (f) {
	  ...
	  return function () {
	    ...
	    nextTask = nextTask.then(
	      function (win) { return f.apply(this$1, arguments$1); },
	      function (fail) { return f.apply(this$1, arguments$1); }
	    );
	    return nextTask
	  }
	}
	...
	function getUserId (localStorage, username) {
	  ...
	  return scrypt(username, userIdSnrp).then(function (userId) { return userId.toString('base64'); })
	}
	...
	function any (promises) {
	  return new Promise(function (resolve, reject) {
	    ...
	    for (var i = 0, list = promises; i < list.length; i += 1) {
	      ...
	      promise.then(
	        function (value) { return resolve(value); },
	        function (error) { return --pending || reject(error); }
	      );
	    }
	  })
	}
	 ...
	 */
	function Login (localStorage, username, userId, dataKey) {
	  // Identity:
	...
	Login.online = function (localStorage, username, userId, dataKey, loginReply) {
	  insert(localStorage, username, userId);
	...
	function loginOffline (ctx, username, userId, password) {
	  ...
	  // Decrypt the dataKey:
	  return scrypt(username + password, passwordKeySnrp).then(function (passwordKey) {
	...
	function loginOnline (ctx, username, userId, password) {
	  return scrypt(username + password, passwordAuthSnrp).then(function (passwordAuth) {
	    // Encode the username:
	    var request = {
	      'userId': userId,
	...
	function login (ctx, username, password) {
	  ...
	  return getUserId(ctx.localStorage, username).then(function (userId) {
	 ...
	 */
	function makeSetup (ctx, dataKey, username, password) {
	  var up = username + password;

	  // dataKey chain:
	  var boxPromise = makeSnrp().then(function (passwordKeySnrp) {
	...
	function setup (ctx, login, password) {
	  return makeSetup(ctx, login.dataKey, login.username, password).then(function (setup) {
	 ...
	 */
	function makeSetup$1 (ctx, login, pin) {
	...
	function setup$1 (ctx, login, pin) {
	  var setup = makeSetup$1(ctx, login, pin);
	 ...
	 */
	function makeSetup$2 (ctx, login, questions, answers) {
	...
	function setup$2 (ctx, login, questions, answers) {
	  var setup = makeSetup$2(ctx, login, questions, answers);
	...
	function create (ctx, username, password, opts) {
	  ...
	  return Promise.all([
	    getUserId(ctx.localStorage, username),
	    makeSetup(ctx, dataKey, username, password)
	    ...
	    return ctx.authRequest('POST', '/v1/account/create', request).then(function (reply) {
	      ...
	      var login$$1 = Login.offline(ctx.localStorage, username, userId, dataKey);

	      // Now activate:
	      var request = login$$1.authJson();
	      return ctx.authRequest('POST', '/v1/account/activate', request).then(function (reply) { return login$$1; })
	    })
	  })
	}
	...
	exports.makeABCContext = makeContext;
...
/***/ function(module, exports, __webpack_require__) {
	...
	hash.ripemd = __webpack_require__(11);
	hash.hmac = __webpack_require__(12);
...
/***/ function(module, exports, __webpack_require__) {
	...
	elliptic.version = __webpack_require__(19).version;
	...
	// Protocols
	elliptic.ec = __webpack_require__(33);
	elliptic.eddsa = __webpack_require__(36);
	...
	module.exports = {
		"_args": [
			[
				...
				"/home/bojack/ab/airbitz-core-js"
			]
		],
		...
		"_where": "/home/bojack/ab/airbitz-core-js",
		"author": {
			"name": "Fedor Indutny",
			"email": "fedor@indutny.com"
		},
...
/***/ function(module, exports, __webpack_require__) {
	...
	curve.base = __webpack_require__(27);
	curve.short = __webpack_require__(28);
	curve.mont = __webpack_require__(29);
	curve.edwards = __webpack_require__(30);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var KeyPair = __webpack_require__(34);
