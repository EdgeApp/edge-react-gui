...
// unit tests and by `scripts/splitEnvJson.ts` (`npm run split-env-json`).
  ...
  for (const [field, value] of Object.entries(env)) {
    ...
    if (field === 'RAMP_PLUGIN_INITS') {
      for (const [id, rampValue] of Object.entries(asMap(value))) {
        ...
        // Keep an empty object: some ramps are enabled with defaults and ship as
        // `{}` in env.json. Dropping them made the golden round-trip fail and
        // would omit the plugin from config.rampPlugins entirely.
        if (isPlainObject(rampValue)) {
          config.rampPlugins[id] = isPlainObject(cfg) ? cfg : {}
        } else {
