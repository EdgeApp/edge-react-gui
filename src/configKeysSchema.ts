...
export const globalKeysShape = {
  ...
  WALLETCONNECT_PROJECT_ID: asOptional(asString, ''),
...
/**
 * The same partner fields, accepted flat at the top of `keys.json` for legacy
 * on-disk files. No defaults: an absent flat field stays absent, so the
 * `cleaner-config` round-trip in `scripts/configure.ts` never writes a flat
 * duplicate of every `globalKeys` entry back into `keys.json`.
 */
type GlobalKeysShape = typeof globalKeysShape
const legacyFlatKeysShape = Object.fromEntries(
  Object.entries(globalKeysShape).map(([key, cleaner]) => [
    key,
    asOptional(cleaner)
  ])
) as {
  [K in keyof GlobalKeysShape]: Cleaner<
    ReturnType<GlobalKeysShape[K]> | undefined
  >
}

/**
 * Cleaner for the private `keys.json` file. Flat partner-key fields are still
 * accepted for legacy on-disk files; `nestGlobalKeys` nests them under
...
export const asKeysJson = asObject({
  ...
  // Auth + local-only telemetry (never served via signed infoRollup):
